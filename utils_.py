...
def read_filtered_eegdata(folder, identifier, freq_band='joint', object_type='pandas_dataframe'):
    ...
    try:
        if freq_band.lower() in ['alpha', 'beta', 'gamma', 'delta', 'theta']:
            ...
            raise ValueError(
                f"Invalid frequency band: {freq_band}. Choose from 'alpha', 'beta', 'gamma', 'delta', 'theta', or 'joint'.")

    except FileNotFoundError:
        raise FileNotFoundError(
            f"File not found for '{identifier}' and frequency band '{freq_band}'. Check the path and file existence.")
...
def load_cms_seed(experiment, feature='PCC', band='joint', imshow=True):
    ...
    path_data = os.path.join(path_parent_parent, 'Research_Data', 'SEED', 'functional connectivity', feature,
                             f"{experiment}.mat")
...
def load_cfs_seed(experiment, feature='de_LDS', band='joint'):
    ...
    path_data = os.path.join(path_parent_parent, 'Research_Data', 'SEED', 'channel features', feature,
                             f"{experiment}.mat")
